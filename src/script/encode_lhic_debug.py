...
def encode_file(
    *,
    ...
    with torch.no_grad():
        start_ch = torch.zeros(2).to(device)
        loss_msp = 0
        # loss_lsp = 0
        # stage param
        param_list = []
        for i in range(B//2):
            param_msp = msp_arm(x_in[i*2:i*2+2], start_ch)
            param_list.append(param_msp)
    
    # 获得全部mu
    param_mu = torch.cat(param_list,dim=0)
            # loss_lsp += lsp_arm(x_in[i*2:i*2+2], start_ch).item() 
    # print("msp loss:",loss_msp/(B//2))
    # print("lsp loss:",loss_lsp/(B//2))
    ...
    param_mu_enc = torch.zeros_like(param_mu, device=device)
